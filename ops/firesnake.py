...
class Snake:
    ...
    def _align_SBS(
        ...
        if method == "DAPI":
            ...
            if align_channels is None:
                raise ValueError(
                    "`align_channels` must be a slice object or list of indices "
                    'when `method`=="SBS_mean", not None.'
                )
            # calculate cycle offsets using the average of SBS channels
            target = Align.apply_window(aligned[:, align_channels], window=window).max(
                axis=1
            )
    ...
    @staticmethod
    def _segment_nuclei_stack(
        dapi, threshold, area_min, area_max, smooth=1.35, radius=15
    ):
    ...
    @staticmethod
    def _annotate_SBS_extra(
        log, peaks, df_reads, barcode_table, sbs_cycles, shape=(1024, 1024)
    ):
        ...
        base_labels = f(df_reads.query("mapped"), footprint=notch)
        base_labels += f(df_reads.query("~mapped"), footprint=plus)
        # Q_min converted to 30 point integer scale
        Q_min = ops.annotate.annotate_points(df_reads, "Q_min", footprint=top_right)
        Q_30 = (Q_min * 30).astype(int)
        # a "donut" around each peak indicating the peak intensity
        peaks_donut = skimage.morphology.dilation(peaks, footprint=np.ones((3, 3)))
    ...
    def _extract_phenotype_FR(data_phenotype, nuclei, wildcards):
        ...
        return Snake._extract_features(
            data_phenotype, nuclei, wildcards, features_frameshift
        ).rename(columns={"label": "cell"})
    ...
    def _extract_phenotype_FR_myc(data_phenotype, nuclei, wildcards):
        ...
        return Snake._extract_features(
            data_phenotype, nuclei, wildcards, features_frameshift_myc
        ).rename(columns={"label": "cell"})
    ...
    def _extract_phenotype_translocation(data_phenotype, nuclei, cells, wildcards):
        ...
        df_n = Snake._extract_features(
            data_phenotype, nuclei, wildcards, features_n
        ).rename(columns={"area": "area_nuclear"})

        df_c = (
            Snake._extract_features(data_phenotype, cells, wildcards, features_c)
            .drop(["i", "j"], axis=1)
        ...
        df = pd.concat(
            [df_n.set_index("label"), df_c.set_index("label")], axis=1, join="inner"
        ).reset_index()
    ...
    def _extract_phenotype_translocation_live(data, nuclei, wildcards):
        def _extract_phenotype_translocation_simple(data, nuclei, wildcards):
            ...
            return Snake._extract_features(data, nuclei, wildcards, features).rename(
                columns={"label": "cell"}
            )
    ...
    @staticmethod
    def _extract_phenotype_translocation_ring(
        data_phenotype, nuclei, wildcards, width=3
    ):
        ...
        return Snake._extract_phenotype_translocation(
            data_phenotype, inner_ring, perimeter, wildcards
        ).rename(columns={"label": "cell"})

    @staticmethod
    def _extract_phenotype_minimal(data_phenotype, nuclei, wildcards):
        return Snake._extract_features(
            data_phenotype, nuclei, wildcards, dict()
        ).rename(columns={"label": "cell"})
    ...
    @staticmethod
    def _analyze_single(
        data, alignment_ref, cells, peaks, threshold_peaks, wildcards, channel_ix=1
    ):
    ...
    def _track_live_nuclei(nuclei, tolerance_per_frame=5):
        ...
        G = df_nuclei.rename(columns={"cell": "label"}).pipe(
            ops.timelapse.initialize_graph
        )
    ...
    @staticmethod
    def _merge_sbs_phenotype(
        sbs_tables, phenotype_tables, barcode_table, sbs_cycles, join="outer"
    ):
        ...
        barcode_to_prefix = lambda x: "".join(x[c - 1] for c in sbs_cycles)
        df_barcodes = barcode_table.assign(
            prefix=lambda x: x["barcode"].apply(barcode_to_prefix)
        ...
        return df_combined.join(barcode_info, on="cell_barcode_0").join(
            barcode_info.rename(columns=lambda x: x + "_1"), on="cell_barcode_1"
        )
    ...
    @staticmethod
    def _summarize_paramsearch_reads(
        barcode_table, reads_tables, cells, sbs_cycles, figure_output
    ):
        import matplotlib
        ...
        df_reads = pd.concat(
            [
                df.assign(total_cells=cell_count)
                for cell_count, (_, df) in zip(
                    n_cells, df_reads.groupby(["well", "tile"], sort=False)
                )
            ]
        )
        df_reads["mapped"] = df_reads["barcode"].isin(barcodes)
            ...
            return pd.Series(
                {
                    ...
                    "mapped_reads_within_cells": df.query("cell!=0")[
                        "mapped"
                    ].value_counts()[True],
                    "mapping_rate": df["mapped"].value_counts(normalize=True)[True],
                    "mapping_rate_within_cells": df.query("cell!=0")[
                        "mapped"
                    ].value_counts(normalize=True)[True],
                    "average_reads_per_cell": df.query("cell!=0").pipe(len)
                    / df.iloc[0]["total_cells"],
                    "average_mapped_reads_per_cell": df.query(
                        "(cell!=0)&(mapped)"
                    ).pipe(len)
                    / df.iloc[0]["total_cells"],
                    "cells_with_reads": df.query("(cell!=0)")["cell"].nunique(),
                    "cells_with_mapped_reads": df.query("(cell!=0)&(mapped)")[
                        "cell"
                    ].nunique(),
                }
            )
        ...
        df_summary = (
            df_reads.groupby(["well", "tile", "THRESHOLD_READS"])
            .apply(summarize)
            .reset_index()
        )
        ...
        sns.lineplot(
            data=df_summary,
            x="THRESHOLD_READS",
            y="average_mapped_reads_per_cell",
            color="steelblue",
            ax=axes[1],
        )
        sns.lineplot(
        ...
        axes[0].legend(
            handles=axes[0].get_lines() + axes_right[0].get_lines(),
            labels=[
                "mapping rate,\nall reads",
                "mapping rate,\nwithin cells",
                "all mapped reads",
        ...
        axes[1].legend(
            handles=axes[1].get_lines() + axes_right[1].get_lines(),
            labels=[
                "mean\nreads per cell",
                "mean mapped\nreads per cell",
                "cells with reads",
...
def load_well_tile_list(filename, include="all"):
    ...
    if include == "all":
        ...
        df_wells_tiles["well_tile"] = df_wells_tiles["well"] + df_wells_tiles[
            "tile"
        ].astype(str)
        include_wells_tiles = ["".join(map(str, well_tile)) for well_tile in include]
        wells, tiles = df_wells_tiles.query("well_tile==@include_wells_tiles")[
            ["well", "tile"]
        ].values.T
...
def input_files(suffix, cycles, directory="input", magnification="10X"):
    ...
    pattern = (
        f"{directory}/{magnification}_{{cycle}}/"
    ...
    if config["MODE"] == "paramsearch_segmentation":
        if isinstance(config["THRESHOLD_DAPI"], list):
            ...
            thresholds_dapi = np.arange(
                config["THRESHOLD_DAPI"] - 200,
                config["THRESHOLD_DAPI"] + 300,
                100,
                dtype=int,
            )
        ...
        if isinstance(config["THRESHOLD_CELL"], list):
            ...
            thresholds_cell = np.arange(
                config["THRESHOLD_CELL"] - 200,
                config["THRESHOLD_CELL"] + 300,
                100,
                dtype=int,
            )

        df_nuclei_segmentation = pd.DataFrame(
            [
                {
                    "THRESHOLD_DAPI": t_dapi,
                    "NUCLEUS_AREA_MIN": n_area_min,
                    "NUCLEUS_AREA_MAX": n_area_max,
                }
                for t_dapi, (n_area_min, n_area_max) in product(
                    thresholds_dapi, nucleus_areas
                )
            ]
        )
        ...
        nuclei_segmentation_paramspace = Paramspace(
            df_nuclei_segmentation,
            filename_params=["THRESHOLD_DAPI", "NUCLEUS_AREA_MIN", "NUCLEUS_AREA_MAX"],
        )

        cell_segmentation_paramspace = Paramspace(
            df_cell_segmentation, filename_params=["THRESHOLD_CELL"]
        )

        config["REQUESTED_FILES"] = []
        config["REQUESTED_TAGS"] = [
            f"segmentation_summary.{nuclei_segmentation_instance}."
        ...
        config["TEMP_TAGS"] = [
            f"nuclei.{nuclei_segmentation_paramspace.wildcard_pattern}.tif",
        ...
        config["REQUESTED_FILES"] = [
            "paramsearch_read-calling.summary.csv",
            "paramsearch_read-calling.summary.pdf",
        ]
        config["REQUESTED_TAGS"] = []
        config["TEMP_TAGS"] = [
            f"bases.{read_calling_paramspace.wildcard_pattern}.csv",
