...
def call_cells(df_reads):
    ...
    # Safeguard against string literal constants used by the library
    cols = [WELL, TILE, CELL]

    # 1. Calculate the ranked barcode frequencies per cell
    # We use a flat reset_index structure to guarantee compatibility
    counts_df = (
    ...
    # Sort securely so rank extraction is predictable
    counts_df = counts_df.sort_values(
        cols + ["count"], ascending=[True, True, True, False]
    )

    # Create a clean grouping object from our flat frame
    s = counts_df.groupby(cols)

    # 2. Extract top (0) and second-top (1) barcodes and their counts securely
    nth_0 = s.nth(0).reset_index()
    nth_1 = s.nth(1).reset_index()
    total_counts = s["count"].sum().reset_index(name=BARCODE_COUNT)

    # Isolate just the target columns to prevent merge collisions
    barcode_0 = nth_0[cols + [BARCODE, "count"]].rename(
        columns={BARCODE: BARCODE_0, "count": BARCODE_COUNT_0}
    )
    barcode_1 = nth_1[cols + [BARCODE, "count"]].rename(
        columns={BARCODE: BARCODE_1, "count": BARCODE_COUNT_1}
    )

    # 3. Base dataframe preparation
    base_df = df_reads.drop_duplicates(cols).query(
        "cell > 0"
    )  # filter reads not in a cell early

    # 4. Sequential merging (Replacing the brittle .join approach)
    result = (
        base_df.merge(barcode_0, on=cols, how="left")
        .merge(barcode_1, on=cols, how="left")
        .merge(total_counts, on=cols, how="left")
    )

    # 5. Fill missing counts with 0 (for cells with fewer than 2 distinct barcodes)
    result[BARCODE_COUNT_0] = result[BARCODE_COUNT_0].fillna(0)
    result[BARCODE_COUNT_1] = result[BARCODE_COUNT_1].fillna(0)

    # 6. Final cleaning operations exactly matching the original pipeline spec
    # Use errors='ignore' in case some coordinate columns were omitted upstream
    drop_cols = [READ, BARCODE, POSITION_I, POSITION_J]
    result = result.drop(
        columns=[c for c in drop_cols if c in result.columns], errors="ignore"
    )

    # Drop columns starting with Q_ (quality scores)
    result = result.filter(regex="^(?!Q_)")
...
def call_cells_mapping(df_reads, df_pool=None):
    ...
    if df_pool is not None:
        ...
        # 1. Map microscope reads to shortened design prefixes
        df_mapped = (
            pd.merge(
                df_reads,
                df_pool[[PREFIX]],
            ...
            .drop(columns=[PREFIX], errors="ignore")
        )
        extra_col = ["mapped"]
    else:
        df_mapped = df_reads.copy()
        extra_col = []

    # 2. Group data safely using clean flat-frame structures
    # We sort by extra_col (mapped) and count descending to prioritize real sgRNAs
    counts_df = (
    ...
    sort_order = cols + extra_col + ["count"]
    ascending_flags = [True] * len(cols) + [False] * (len(extra_col) + 1)
    counts_df = counts_df.sort_values(sort_order, ascending=ascending_flags)

    s = counts_df.groupby(cols)

    # 3. Extract the top (0) and runner-up (1) barcode rows securely
    nth_0 = s.nth(0).reset_index()
    nth_1 = s.nth(1).reset_index()
    total_counts = s["count"].sum().reset_index(name=BARCODE_COUNT)
    ...
    # 4. Filter base frame down early
    base_df = df_reads.drop_duplicates(cols).query("cell > 0")

    # 5. Flat merge instead of brittle index-matching .join()
    df_cells = (
        base_df.merge(barcode_0, on=cols, how="left")
        .merge(barcode_1, on=cols, how="left")
        .merge(total_counts, on=cols, how="left")
    ...
    # Fill NaN values for solitary cell reads with 0
    df_cells[BARCODE_COUNT_0] = df_cells[BARCODE_COUNT_0].fillna(0)
    df_cells[BARCODE_COUNT_1] = df_cells[BARCODE_COUNT_1].fillna(0)

    # 6. Metadata cleanup matching original pipeline spec
    drop_cols = [READ, BARCODE, POSITION_I, POSITION_J]
    df_cells = df_cells.drop(
        columns=[c for c in drop_cols if c in df_cells.columns], errors="ignore"
    )
    df_cells = df_cells.filter(regex="^(?!Q_)")

    # 7. If pool exists, enrich BARCODE_0 and BARCODE_1 with genetic annotations
    if df_pool is not None:
        # Map metadata for dominant barcode (Barcode 0)
        df_cells = (
            pd.merge(
                df_cells,
            ...
            .rename(columns={col: col + "_0" for col in guide_info_cols})
            .drop(columns=[PREFIX], errors="ignore")
        )

        # Map metadata for runner-up barcode (Barcode 1)
        df_cells = (
            pd.merge(
                df_cells,
                df_pool[[PREFIX] + guide_info_cols],
            ...
            .rename(columns={col: col + "_1" for col in guide_info_cols})
            .drop(columns=[PREFIX], errors="ignore")
        )
...
def call_barcodes(df_bases, Y, cycles=12, channels=4):
    ...
    return df_reads.assign(Q_min=lambda x: x.filter(regex="Q_\d+").min(axis=1)).drop(
...
def add_clusters(
    ...
    df_cells[CLUSTER_SIZE] = df_cells.groupby(CLUSTER)[barcode_col].transform("size")
...
def join_by_cell_location(df_cells, df_ph, max_distance=4):
    ...
    return (
        df_cells.assign(cell_ph=cell_ph, distance=distance)
